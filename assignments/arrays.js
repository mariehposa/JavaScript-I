...
let lastCar = 0;
console.log('The last car in your inventory is of ' + inventory[inventory.length - 1].car_make + ' make and ' + inventory[inventory.length - 1].car_model + ' model');
...
for(a = 0; a < inventory.length; a++){
  carModelsSorted = carModelsSorted.concat(inventory[a].car_model);
}
...
for(b = 0; b < inventory.length; b++){
  carYears = carYears.concat(inventory[b].car_year);
}
...
for (let a = 0; a < inventory.length; a++) {
  ...
  if(car.car_make === 'Audi' || car.car_make === 'BMW'){
    BMWAndAudi = BMWAndAudi.concat(car)
  }
